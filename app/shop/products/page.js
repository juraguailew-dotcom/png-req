...
export default function ShopProductsPage() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      // Uses /api/shop/products which already scopes to the caller's shop
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <a href="/shop/inventory/add" className="inline-flex items-center justify-center rounded-lg bg-blue-600 px-6 py-3 text-white hover:bg-blue-700">
            Add Product
          </a>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {products.map((product) => (
              <div key={product.id} className="bg-white rounded-lg shadow hover:shadow-lg transition p-5">
                <div className="bg-gray-100 h-40 rounded-lg mb-5 overflow-hidden">
                  {product.images?.[0] ? (
                ...
                <div className="mt-5 flex gap-2">
                  ...
                  <a href={`/shop/inventory/${product.id}`} className="flex-1 px-3 py-2 rounded-lg bg-gray-100 text-gray-700 text-center hover:bg-gray-200">Edit</a>
                </div>
              </div>
            ))}
          </div>
