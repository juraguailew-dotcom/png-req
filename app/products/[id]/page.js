    ...
    <div>
      ...
      {images.length > 1 && (
        <div className="flex gap-2 mt-2">
          {images.map((url, i) => (
            <button key={url} onClick={() => setActive(i)} className={`w-16 h-16 rounded border-2 overflow-hidden ${i === active ? 'border-blue-500' : 'border-transparent'}`}>
...
export default function ProductDetailPage() {
  ...
  const fetchProduct = async (currentUser) => {
    ...
    try {
      ...
      if (currentUser?.app_metadata?.role === 'hardware_shop') {
        setCanEdit(data.product?.shop?.id === currentUser.id);
      } else {
        setCanEdit(false);
      }
    } catch (err) {
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            {canEdit && (
              <a
                ...
                className="inline-flex items-center justify-center rounded-lg bg-gray-100 px-4 py-2 text-gray-800 hover:bg-gray-200"
              >
                Edit Product
              </a>
            )}
          ...
          <div className="lg:col-span-2 bg-white rounded-lg shadow p-6">
            <div className="mb-6">
              {product.images?.length > 0 ? (
                ...
                <div className="bg-gray-100 h-64 rounded-lg flex items-center justify-center text-gray-400">No image</div>
              )}
            </div>
